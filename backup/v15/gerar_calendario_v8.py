"""V8 calendário operacional — modelo V15 ensemble (5 seeds).

Gera calendário usando voto/Q-mean dos 5 modelos V15 treinados.
...
print(f"\nV8 rollout V15 ensemble ({args.mode}, {len(modelos)} seeds) "
...
calendario = {
    'versao': 'V8-modelo-V15-ensemble',
    'modelo_base': f'V15 ensemble de {len(modelos)} seeds ({args.mode}) — PER + curriculum + mask dinâmico',
...
with open(V15 / 'calendario_v8.json', 'w', encoding='utf-8') as f:
    json.dump(calendario, f, indent=2, ensure_ascii=False, default=str)

print()
print("=" * 70)
print(f"CALENDÁRIO V8 GERADO (V15 ensemble, {len(modelos)} seeds)")
print("=" * 70)
...
print()
print(f"✓ Saída: results/v15/calendario_v8.json")

# ── Hook automático: regenera HTML premium ───────────────────────────────
try:
    import subprocess
    import shutil
    # Reusar template HTML do V12 apontando para V15 JSON
    shutil.copy(V15 / 'calendario_v8.json',
                ROOT / 'results' / 'v12' / 'calendario_v4_anual.json')
    print()
    print("Gerando HTML premium V15...")
    ...
    shutil.copy(ROOT / 'results' / 'v12' / 'calendario_premium_v12.html',
                V15 / 'calendario_premium_v15.html')
    print(f"✓ HTML: results/v15/calendario_premium_v15.html")
