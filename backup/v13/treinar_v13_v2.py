"""V13.v2 — BranchingDQNCross + Action Masking.

Híbrido: estrutura V12.1 + cross-head para destravar liq25%/desc10% sem
perder o sinal de eventos comerciais.

Bellman update igual ao V12 (BranchingDQN), só muda a arquitetura.
"""
import argparse
import io
import sys
...
from dqn import BranchingDQNCross, ReplayBuffer, NEG_INF
...
def treinar_seed(seed, n_episodios, max_steps, DEVICE=None):
    ...
    online = BranchingDQNCross(obs_dim, n_p, n_i).to(DEVICE)
    target = BranchingDQNCross(obs_dim, n_p, n_i).to(DEVICE)
        ...
        for step in range(max_steps):
            ...
            if len(buf) >= 1000:
                ...
                with torch.no_grad():
                    ...
                    # Aplica mask antes do argmax (mask por categoria)
...
def main():
    ...
    print(f"V13.{args.suffix} — BranchingDQNCross + Action Mask")
