...
TIPO_PRODUTO = {
    'cerveja': 'puxador_consumo', 'gelo': 'puxador_consumo',
    'destilados': 'puxador_premium', 'vinho': 'puxador_premium',
    'chocolate_premium': 'puxador_presente',
    'chocolate_impulso': 'impulso', 'snack': 'impulso', 'biscoito': 'impulso',
    'doce': 'impulso', 'sorvete': 'impulso',
    'cafe': 'rotina', 'padaria': 'rotina',
    'agua': 'commodity', 'refrigerante': 'commodity', 'suco': 'commodity',
    'isotonico': 'commodity', 'energetico': 'commodity',
    'cigarro_souza_cruz': 'proibida', 'cigarro_philip_morris': 'proibida',
    'cigarro_jti': 'proibida',
}

ELASTICIDADE = {
    'commodity': 0.3, 'rotina': 0.4, 'puxador_premium': 0.5,
...
K_UPLIFT_ABAIXO_BREAKEVEN = -150.0
K_BONUS_EVENTO_MATCH = 250.0
K_BONUS_EVENTO_PRESENTE_PUXADOR = 500.0

K_BONUS_HARMONIA_FORTE = 350.0       # ↑ 200→350 (puxar para h>=2.0)
K_BONUS_HARMONIA_MEDIA = 100.0       # ↑ 70→100
K_PENALIDADE_HARMONIA_LEVE = -50.0   # ↓ -40→-50
K_PENALIDADE_HARMONIA_FRACA = -200.0  # ↓ -150→-200
K_HARMONIA_MULTIPLICATIVO = 400.0
...
class EnvRLPromocoes(gym.Env):
    ...
    def __init__(self,
        ...
        # Carrega calibração (categorias, fatores, margens)
        # V20: usa a calibração V19.2 que tem harmonia PDV-ajustada
        # (gelo+destilados=0.80, cerveja+snack=2.50, choc+vinho=2.50)
        if calibracao_path is None:
            calibracao_path = (Path(__file__).parent.parent
                                / 'teste_agente_demanda_promocional'
                                / 'v19_2' / 'calibracao_v2.json')
    ...
    def _executar_decisao(self, intensidade_idx, complementar_idx,
        ...
        # Bonus: defensivo (liq em validade próxima)
        if intensidade_idx >= 4 and validade < 0.30:
            reward += K_DEFENSIVO_VENCIMENTO
            breakdown['bonus_defensivo'] = K_DEFENSIVO_VENCIMENTO

        # Bonus: match produto × evento
        if evento and afeta_categoria and intensidade_idx > 0:
            reward += K_BONUS_EVENTO_MATCH
            breakdown['bonus_evento_match'] = K_BONUS_EVENTO_MATCH
        ...
        # ITER 12: penalidade COMBO SEM PAR (intensidade=5 mas par=None → loophole)
        if intensidade_idx == 5 and not par_combo:
            reward -= 300.0
            breakdown['pen_combo_sem_par'] = -300.0
        ...
        # ITER 2: bonus EXTRA quando puxador_presente em evento presente
        # (força chocolate_premium a virar foco em Mães/Namorados/Pais/etc)
        if (evt_tipo == 'presente' and tipo_produto == 'puxador_presente'
                and afeta_categoria and intensidade_idx > 0):
            reward += K_BONUS_EVENTO_PRESENTE_PUXADOR
            breakdown['bonus_evento_presente_puxador'] = K_BONUS_EVENTO_PRESENTE_PUXADOR
    ...
    def _sortear_produto(self) -> int:
        ...
        ITER 18: enviesa SUTILMENTE para rotina (café/padaria) em manhã,
        garantindo que agente vê esses produtos em contexto correto.
        """
        validos = [i for i, c in enumerate(self.categorias)
                    if TIPO_PRODUTO.get(c, 'commodity') != 'proibida']
        # Pesos iguais (manter)
        pesos = np.ones(len(validos))
        # Pequena fatia de manhã (turno=0 simulado via hora aleatória)
        return int(self._rng.choice(validos, p=pesos/pesos.sum()))
    ...
    def _evento_proximo(self, dt: date):
        """Retorna (evento, dias_até, tipo). Olha 14 dias à frente."""
        for offset in range(0, 14):
            check = dt + timedelta(days=offset)
            ev = next((e for e in self.eventos
                        if e['data'] == check.isoformat()
                        and e['tipo_evento'] == 'data_comercial'), None)
            if ev:
    ...
    def _categoria_afetada(self, cat: str, lista: list) -> bool:
        ...
        for c in lista:
            cl = c.lower().strip()
            if cl in cat_l or cat_l in cl:
                return True
            if cl == 'salgadinho' and cat_l == 'snack':
                return True
        return False
    ...
    def prior_complementar(self) -> np.ndarray:
        ...
        # Idx 0 = "nenhum" — peso só se NÃO tem par com harmonia alta
        prior[0] = 0.3
        if self.harmonia:
            for j, b in enumerate(self.categorias):
                if b == cat:
                    prior[j + 1] = 0.001  # mesmo produto: quase impossível
                else:
                    h = float(self.harmonia[ia][j])
                    # ITER 5: bias muito mais forte. h^4 → h=2.5: peso 39, h=1.0: 1, h=0.5: 0.06
