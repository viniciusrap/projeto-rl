...
# CENÁRIOS DE VALIDAÇÃO (mesmos da V19.2)
CENARIOS_VALIDACAO = [
    {
        'id': 'C1_gelo_cerveja_fds',
        'categoria': 'gelo', 'data': date(2027, 1, 9),
        'estoque': 1.0, 'validade': 1.0,
        'check_acao_correta': lambda r: r['acao_intensidade'] == 'combo' and r['acao_complementar'] == 'cerveja',
        'descricao': 'Gelo+Cerveja em sáb verão',
    },
    {
    ...
    {
        'id': 'C3_choc_vinho_maes',
        'categoria': 'chocolate_premium', 'data': date(2027, 5, 5),
        'estoque': 1.0, 'validade': 1.0,
        'check_acao_correta': lambda r: r['acao_intensidade'] == 'combo' and r['acao_complementar'] == 'vinho',
        'descricao': 'Chocolate+Vinho Mães',
    },
    {
    ...
    {
        'id': 'C5_choc_impulso_baixa',
        'categoria': 'chocolate_impulso', 'data': date(2026, 9, 14),
        'estoque': 1.0, 'validade': 1.0,
        'check_acao_correta': lambda r: r['acao_intensidade'] in ('nada', 'desc3%', 'desc5%'),
        'descricao': 'Chocolate impulso baixa',
    },
    {
...
def rodar_iteracao(iter_nome, n_episodios=400, seed=0, n_seeds=1):
    ...
    from treinar_v20 import treinar
    model_paths = []
    rewards_finais = []
