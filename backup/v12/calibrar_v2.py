...
constantes = {
    ...
    'K_EVENTO_PRESENTE': 600.0,  # V12.1 baseline (V13.v3 K=1000 falhou)
    ...
    # V16.1 (análise crítica): 70 era baixo demais (agente promoveria ~60%
    # em alta). 120 é o meio-termo correto: cobre lucro extra esperado de
    # promo em alta (~80-150 R$/turno) mas permite ~30% quando há ROI claro.
    'K_DESC_ALTA_SAUDAVEL': 120.0,
    ...
    'DESC_COMBO_MAX': 0.10,  # V16: 5% → 10% (Vinicius: vantagem percebida)
...
HARMONIA_PARES = {
    ...
    ('gelo', 'isotonico'):                  1.5,
    ('gelo', 'sorvete'):                    1.4,
    # ─── Drinks (bar em casa) — calibrados c/ Instacart (lift real) ───
    ('destilados', 'gelo'):                 2.2,
    ('destilados', 'vinho'):                2.5,  # Instacart lift 14.0!
    ('destilados', 'refrigerante'):         2.0,  # Instacart lift 2.02
    ('destilados', 'snack'):                1.6,
    ('destilados', 'isotonico'):            1.5,  # Instacart lift 1.53
    ('destilados', 'suco'):                 1.4,
    # ─── Outros pares validados Instacart ───
    ('isotonico', 'refrigerante'):          1.8,  # Instacart lift 1.63
    ('biscoito', 'chocolate_premium'):      1.3,  # Instacart lift 1.26
    # ─── Café da manhã / tarde ───
    ('biscoito', 'cafe'):                   2.3,
    ('biscoito', 'suco'):                   1.7,
