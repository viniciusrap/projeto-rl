...
class ConvenienceStoreEnvV12(gym.Env):
    ...
    def __init__(self, calibracao_path: str = 'data/calibracao_v2.json',
        ...
        self.turno = 0
        self.passo = 0
        # V14: histórico de categorias promovidas (para mask dinâmico)
        # Lista das últimas K categorias promovidas (0 = não-promo, descartado)
    ...
    def step(self, action):
        ...
        # V14: atualiza histórico de categorias (só efetivas promo)
        if prod_idx > 0 and intensidade > 0:
            self.historico_categorias.append(prod_idx)
            if len(self.historico_categorias) > self.MAX_HISTORICO:
                self.historico_categorias.pop(0)
        self.turno += 1
        if self.turno >= 3:
            self.turno = 0
    ...
    # ── V12.3: Action masking para hard constraints ────────────────────

    def get_action_mask(self, no_repeat_window: int = 2) -> np.ndarray:
        """V14 — Retorna máscara de categorias válidas.

        Shape: (N+1,) boolean. True = categoria pode ser promovida.
        - Índice 0 = "sem promoção" (sempre True)
        - Índices 1..N = categorias promovíveis

        Aplica 2 restrições:
        1. **Regulatório** (Lei 9.294/96): cigarros sempre False
        2. **Anti-saturação** (V14): não promover mesma categoria nas últimas
           `no_repeat_window` decisões de promo efetiva. Força diversidade
           e reflete realidade (cliente cansa de ver mesmo desconto).

        Args:
            no_repeat_window: tamanho da janela de não-repetição (default 2).
                Use 0 para desativar (compat V13).
        """
        mask = np.ones(self.N + 1, dtype=bool)
        # Restrição 1: não-promovíveis (água + 3 cigarros)
        for i, c in enumerate(self.cats):
            if not c.get('promovivel', True):
                mask[i + 1] = False
        # Restrição 2 (V14): janela de não-repetição
        if no_repeat_window > 0 and self.historico_categorias:
            recentes = self.historico_categorias[-no_repeat_window:]
            for cat_idx in recentes:
                if 0 < cat_idx <= self.N:
                    mask[cat_idx] = False
        return mask
