"""Gerar calendário V21 PDV-native — rollout determinístico do agente treinado.

Roda múltiplos episódios em DATAS REAIS do calendário e coleta as decisões.
V21 usa calibração SEM Instacart, harmonia criada por conhecimento de PDV.
"""
import io
import json
import sys
...
def rollout_deterministico(env: EnvRLPromocoes, agent: BranchingDQNAgent,
    ...
    decisoes = []
    # V21 LIMPO: SEM injeção estrutural. O agente RL decide TUDO via reward.
    # Esquenta de Sexta (cerveja+snack em sex/sáb) deve emergir do K_ESQUENTA_SEXTA.
    estruturais = []
    ...
    for d in range(dias):
        ...
        if done:
            ...
            obs = env._observar()
    # Mescla estruturais (Esquenta de Sexta) com decisões do agente
    decisoes.extend(estruturais)
    return decisoes
...
def main():
    ...
    model_path = HERE / 'models' / 'v21_final.pt'   # iter 5 (10/12 PDV)
    out_dir = PROJ_ROOT / 'results' / 'v21'
    ...
    for d in decisoes:
        ...
        reward_d = d.get('reward_final', d.get('reward', 0))
        # V21: estruturais SEMPRE como campanha individual (não agrupa consecutivos)
        eh_estrutural = d.get('origem', '').startswith('ESTRUTURAL_')
        if eh_estrutural and cur:
            campanhas.append(cur)
            cur = None
        if cur and not eh_estrutural and (cur['categoria'], cur['intensidade'], cur['par_combo']) == key:
            cur['dias'] += 1
            cur['lucro_total_acumulado'] += d['lucro_total']
            cur['reward_total_acumulado'] += reward_d
            ...
            cur = {
                ...
                'reward_total_acumulado': reward_d,
    ...
    # Salva JSON
    out = {
        'versao': 'V21 PDV-native — RL Branching DQN com calibração de domínio (sem Instacart)',
    ...
    out_path = out_dir / 'calendario_v21.json'
